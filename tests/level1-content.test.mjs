...
for(const theme of themes){
  ...
  for(const [phase, day] of [['input', theme.inputDay], ['output', theme.outputDay]]){
    ...
    for(const word of day.words){
      ...
      assert.ok(!wordIds.has(word.id), `duplicate word id: ${word.id}`);
      wordIds.add(word.id);
...
assert.equal(wordIds.size, themes.reduce((count, theme) => count + theme.inputDay.words.length + theme.outputDay.words.length, 0));
console.log(`level one part one: ${themes.length} themes and ${wordIds.size} unique words validated`);
